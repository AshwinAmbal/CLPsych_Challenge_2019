...
class CLPsychProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        logger.info("LOOKING AT {}".format(os.path.join(data_dir, "User_Posts_Processed_Train_Final.csv")))
...
def main():
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        outcsv = open("/home/aambalav/Reqd_Labels.csv",'w', encoding = 'utf8', newline='')
        writer = csv.writer(outcsv,quotechar = '"')
        for user,true, pred in zip(complete_user_ids, complete_label_ids, complete_outputs):
            writer.writerow([user,true,pred])
        eval_loss = eval_loss / nb_eval_steps
        eval_loss = eval_loss / nb_eval_steps        
        ...
        print(classification_report(complete_label_ids, complete_outputs))
